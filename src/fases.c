...
void desenha_fase(Jogo * j, int n_fase){
...
void fases_desenha(Jogo * j){
		...
		for (int x=0; x<FASE_LARGURA; x++){
			switch(f->mapa[y][x]){
				case 'c':{
					...
					if (x > FASE_LARGURA/2){
						textura_desenha(j,D_CANO, pos_screen);
					} else{
						textura_desenha(j,E_CANO, pos_screen);
					}
					break;
				}
				case 'b': {
	...
	desenha_pontos(j, f->mario.score);
	desenha_fase(j, f->n_mapa);
